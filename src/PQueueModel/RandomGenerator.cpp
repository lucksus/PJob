...
PQueueMath::RandomGenerator::RandomGenerator()
...
void PQueueMath::RandomGenerator::seed(const qint32 idum){
	this->idum = idum;
	if (idum > 0)
	{
...
void PQueueMath::RandomGenerator::NewRandSeed(){
	struct tm *newtime;
	time_t ltime;
	clock_t cl;
	qint32 cl_int;
...
PQueueMath::RandomGenerator*	PQueueMath::RandomGenerator::getInstance(){
	static PQueueMath::RandomGenerator instance;
	return &instance;
}


double	PQueueMath::RandomGenerator::uniform(double xmin, double xmax){
	return getInstance()->_uniform(xmin,xmax);
}

double	PQueueMath::RandomGenerator::gauss(double mean, double var){
	return getInstance()->_gauss(mean,var);
}

double	PQueueMath::RandomGenerator::poisson(double lambda){
	return getInstance()->_poisson(lambda);
}

double	PQueueMath::RandomGenerator::fastpoisson(double lambda){
	return getInstance()->_fastpoisson(lambda);
}



void	PQueueMath::RandomGenerator::setDeterministic(const qint32 idum){
	getInstance()->seed(idum);
	getInstance()->gaussAlreadyComputed = false;
}

void PQueueMath::RandomGenerator::setDeterministic() {
	setDeterministic( getDefaultSeed() );
}

void PQueueMath::RandomGenerator::setDefaultSeed(const qint32 idum) {
	if (idum > 0)
	{
//		logOut.warning("RandomGenerator: seed is positive, but must be negative. Setting seed = -seed");
...
qint32 PQueueMath::RandomGenerator::getDefaultSeed() {
	return getInstance()->m_defaultIdum;
}


void	PQueueMath::RandomGenerator::setRandom(){
	getInstance()->NewRandSeed();
}


double PQueueMath::RandomGenerator::_uniform(double min_wert, double max_wert){
...
double PQueueMath::RandomGenerator::_gauss(const double mean, const double var){
...
double PQueueMath::RandomGenerator::_poisson(const double mean){
	double x, y;
	double z;
...
double PQueueMath::RandomGenerator::_fastpoisson(const double lambda){
...
double PQueueMath::RandomGenerator::_gammln(const double xx){
	double x,y,tmp,ser;
...
double PQueueMath::RandomGenerator::_pareto(double MinValue, double MeanValue){
...
double PQueueMath::RandomGenerator::pareto(double MinValue, double MeanValue){
...
double PQueueMath::RandomGenerator::_shuffle(qint32 *idum)
{
	int j;
	qint32 k;
	static qint32 iy=0;
...
double PQueueMath::RandomGenerator::_negExp(double lambda)
{
	double U = _shuffle(&idum);
	while (U == 0) U = _shuffle(&idum);
...
double PQueueMath::RandomGenerator::negExp(double lambda){
	return getInstance()->_negExp(lambda);
}


void PQueueMath::RandomGenerator::initFromSimulationParameters(bool isDeterministic, bool isTransientCalc=false){
	if (isTransientCalc)
	{
